...
def get_dataset(dataset_name,
    ...
    def preprocess_and_tokenize(example):
        if dataset_name == 'leandojo':
            ...
            goal_text = example['goal']  # + '\n' + example['proof']
            proof_text = example['proof']

            if detokenizer is not None:
                goal_text = _apply_detokenizer(detokenizer)(goal_text)
                proof_text = _apply_detokenizer(detokenizer)(proof_text)
            ...
            tokens = {'input_ids': [[BOS] + goal_tokens['input_ids'][i] + proof_tokens['input_ids'][i] + [EOS] * 10 for i in # add 2 pad to teach model to stop generating
...
def get_dataloaders(config, tokenizer, skip_train=False,
    ...
    print(config.loader.global_batch_size, config.loader.batch_size, config.trainer.num_nodes, num_gpus,
