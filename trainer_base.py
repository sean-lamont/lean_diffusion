    ...
    def on_after_backward(self) -> None:
        for name, p in self.named_parameters():
            if p.grad is not None and torch.isnan(p.grad).any():
                print(f'Gradient is NaN for parameter: {name}')
                # Optionally, you can set the gradient to zero
...
class UniformState(Diffusion):
    ...
    def q_xt(self, x, alpha_t):
        ...
        # todo add conditional generation here? i.e. keep all context tokens from original x
        # todo also consider doing this in a similar way to process_model_output of mdlm
        if self.ignore_bos:
            xt[:, 0] = x[:, 0]
        return xt
